...
#view-formacion .oc-react-card{
  min-width:0;background:#121824;border:1px solid rgba(255,255,255,.12);
  border-radius:18px;overflow:hidden;color:#fff;text-decoration:none;
  display:flex;flex-direction:column;
  transition:transform .24s ease,border-color .24s ease,box-shadow .24s ease;
}
#view-formacion .oc-react-card:hover{
  transform:translateY(-4px);border-color:rgba(24,183,241,.6);
  box-shadow:0 18px 38px rgba(0,0,0,.22)
}
#view-formacion .oc-react-media{height:245px;background:#0e1420;overflow:hidden;position:relative}
#view-formacion .oc-react-media img{
  width:100%!important;height:100%!important;display:block!important;
  opacity:1!important;visibility:visible!important;object-fit:cover!important;
  object-position:center 30%!important
}
#view-formacion .oc-react-level{
  position:absolute;z-index:2;left:14px;top:14px;padding:7px 10px;
  border-radius:8px;background:#d52b3d;color:#fff;font-size:.55rem;
  font-weight:900;letter-spacing:.05em
}
#view-formacion .oc-react-body{
  padding:17px 17px 18px;display:flex;flex-direction:column;min-height:165px
}
#view-formacion .oc-react-body>small{
  color:#d52b3d!important;font-size:.56rem;font-weight:900;letter-spacing:.1em
}
#view-formacion .oc-react-body h3{
  margin:7px 0 6px;color:#fff!important;font-size:1.02rem;line-height:1.2
}
#view-formacion .oc-react-body p{
  margin:0;color:rgba(255,255,255,.66)!important;font-size:.72rem;line-height:1.5;flex:1
}
#view-formacion .oc-react-meta{
  margin-top:13px;color:#b9dff0!important;font-size:.64rem;font-weight:700
}
...
export function initOpenCourseFormation() {
  ...
  if (title) title.innerHTML = 'Elige una ruta. <span class="oc-blue">Aprende por módulos.</span>';
  ...
  grid.innerHTML = COURSES.map((course) => `
    <a class="oc-react-card" href="course.html?course=${course.id}" target="_blank" rel="noopener">
      <div class="oc-react-media">
        <img src="${course.image}" alt="${course.title}" loading="eager" decoding="async">
